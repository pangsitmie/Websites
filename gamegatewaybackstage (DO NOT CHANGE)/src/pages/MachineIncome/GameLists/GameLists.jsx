...
import style from "./GameLists.module.scss";
...
export default () => {
    ...
    const splitPage = () => {
        let lists = []
        for (let id in physicalMachineMarketings) {
            if (lists.length === 10) {
                pages.push(lists)
                lists = []
            }
    ...
    if (pages.length > 0){
        pages[state.gamePageInfo - 1].map(async value => {
                ...
                gameID_lists.push(
                    <div key={'tag' + value} className={style.lists} id='lists'>
                        ...
                        <span>
                            <button
                                onClick={async () => {
                                    putProvider('PUT_GAMESERIALID', value)
    ...
    const nextPage = (
        <button onClick={() => putProvider('PUT_GAMEPAGEINFO', state.gamePageInfo + 1)}>
            {language.nextPage}
        </button>
    )
    const backPage = (
    ...
    gameID_lists.push(
        <div key='return' className={style.pages}>
            {state.gamePageInfo > 1 ? backPage : ''}
            {state.gamePageInfo < pages.length ? nextPage : ''} 
        </div>
    )
